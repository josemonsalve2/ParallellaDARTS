...
DEFINE_TP_MEM_REGIONS(finish_tp,
                          // DRAM
                          ,// INTERNAL
                          ,// DIST
                     )

DEFINE_THREADED_PROCEDURE(finish_tp,1, {
                    e_darts_print("Initializing a finishingTp TP \n");
                    ASSIGN_SYNC_SLOT_CODELET(*this,0,finishExecution,1,1,1);
                    syncSlot_t * finish = GET_SYNC_SLOT(*this, 0);
                    syncSlotDecDep(finish);
                    });

DEFINE_TP_CLOSURE(finish_tp);
...
void finishExecution()
{
    *finalSignal = 1; // Make the pseudo runtime stop pulling from the queue
}
